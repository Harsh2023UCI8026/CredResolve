...
export default function SmartDialerDashboard() {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 font-sans antialiased ${
      ...
      >
        {/* Top Mobile Row / Left Desktop Row: Phone Logo, Title, Badge & Theme Toggle */}
        <div className="flex items-center justify-between w-full sm:w-auto gap-2">
          <div className="flex items-center space-x-2 sm:space-x-3 min-w-0">
            ...
            <div className="min-w-0 flex flex-col justify-center space-y-0.5">
              <div className="flex items-center gap-1.5 sm:gap-2">
                <h1 className="text-base sm:text-lg font-black tracking-tight leading-none text-slate-900 dark:text-white shrink-0">
                  SmartDialer
                </h1>
          ...
          {/* Theme Toggle on Top Right for Mobile */}
          <button
            onClick={toggleTheme}
            aria-label={`Switch to ${isDark ? 'Light' : 'Dark'} mode`}
            title={`Switch to ${isDark ? 'Light' : 'Dark'} mode`}
            className={`touch-target p-2 rounded-xl border flex sm:hidden items-center justify-center transition-all shrink-0 ${
              isDark 
        ...
        {/* Action Buttons Row: Full text labels 'How it Works?' & 'API & Docs Hub' with equal flex expansion and zero overlap */}
        <div className="flex items-center justify-between sm:justify-end space-x-2 sm:space-x-3 w-full sm:w-auto shrink-0">
          <button
            ...
            className={`touch-target flex-1 sm:flex-initial px-3 py-2 sm:px-3.5 sm:py-2 rounded-xl text-xs font-extrabold flex items-center justify-center gap-1.5 border transition-all ${
              isDark 
          ...
          >
            <Code2 className="w-4 h-4 text-orange-200 shrink-0" />
            <span className="whitespace-nowrap">API &amp; Docs Hub</span>
            <ExternalLink className="w-3.5 h-3.5 opacity-80 shrink-0 hidden sm:inline" />
          </button>
          ...
          <button
            ...
            aria-label={`Switch to ${isDark ? 'Light' : 'Dark'} mode`}
            title={`Switch to ${isDark ? 'Light' : 'Dark'} mode`}
            className={`touch-target p-2 sm:p-2.5 rounded-xl border hidden sm:flex items-center justify-center transition-all shrink-0 ${
              isDark 
                ? 'bg-zinc-800 hover:bg-zinc-700 text-amber-300 border-zinc-700 shadow-md' 
                : 'bg-amber-100 hover:bg-amber-200 text-amber-900 border-amber-300 shadow-sm'
            }`}
